...
void ZipFile(void* input)
{
    ...
    LogTrace("Zipping file \"%s\" on thread %ld ...", fcInfo->FilePath, pthread_self());
...
void ZipFiles()
{
	...
	// Create threadpool
	struct ThreadPool* threadPool = ThreadPool_Create(MAX_PROCESS_COUNT);
	
	// Add each file to threadpool
	int iFCInfo;
	for(iFCInfo = 0; iFCInfo < InputFileCount; iFCInfo++)
    {
		...
		// Add file to threadpool
        LogTrace("Zipping file \"%s\" ...", fcInfo->FilePath);
		ThreadPool_AddWork(threadPool, ZipFile, fcInfo);
	}
